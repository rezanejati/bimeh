...
import library.android.calendar.mohamadamin.persianmaterialdatetimepicker.date.MonthView.OnDayClickListener;
...
public abstract class MonthAdapter extends BaseAdapter implements OnDayClickListener {
    ...
    private CalendarDay mEndSelectedDay;
    ...
    public MonthAdapter(Context context,
        ...
        setEndSelectedDay(mController.getEndSelectedDay());
    }
    ...
    public void setEndSelectedDay(CalendarDay day) {
        mEndSelectedDay = day;
        notifyDataSetChanged();
    }

    protected boolean isEndSelected() {
        return mEndSelectedDay != null;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        int selectedDay = -1;
        int endSelectedDay = -1;
        if (isSelectedDayInMonth(year, month)) {
            selectedDay = mSelectedDay.day;
        }
        if (isEndSelectedDayInMonth(year, month)) {
            endSelectedDay = mEndSelectedDay.day;
        } else if (isEndSelectedDayGreaterThanMonth(year, month)) {
            endSelectedDay = 32;
        }
        ...
        drawingParams.put(MonthView.VIEW_PARAMS_END_SELECTED_DAY, endSelectedDay);
    ...
    private boolean isEndSelectedDayGreaterThanMonth(int year, int month) {
    ...
    private boolean isEndSelectedDayInMonth(int year, int month) {
        if (mEndSelectedDay == null) return false;
        return mEndSelectedDay.year == year && mEndSelectedDay.month == month;
    }
    ...
    protected void onDayTapped(CalendarDay day) {
        if (!isStartDateSelected) {
            if(mController.isEndSelectable()) isStartDateSelected = !isStartDateSelected;
            mController.tryVibrate();
            mController.onDayOfMonthSelected(day.year, day.month, day.day);
            setSelectedDay(day);
            setEndSelectedDay(null);
        } else {
            if (day.compareTo(mSelectedDay) < 0) {
                mController.tryVibrate();
                mController.onDayOfMonthSelected(day.year, day.month, day.day);
                setSelectedDay(day);
                setEndSelectedDay(null);
            ...
            mController.tryVibrate();
            mController.onEndDayOfMonthSelected(day.year, day.month, day.day);
